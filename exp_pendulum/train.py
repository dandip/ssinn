...
from function_spaces import gradient_wrapper, univariate_trinomial, univariate_fivenomial, univariate_trigonometric_term, univariate_trig_tri_mixed
...
def get_args():
    ...
    parser.add_argument('--tp', default=0, type=int, help='function space for tq: 0 (trinomial), 1 (trigonometric), 2 (mixed), 3 (fivenomial)')
    parser.add_argument('--vq', default=1, type=int, help='function space for vq: 0 (trinomial), 1 (trigonometric), 2 (mixed), 3 (fivenomial)')
    ...
    parser.add_argument('--n_epochs', default=40, type=int, help='epochs')
    parser.add_argument('--regularization', default=1e-4, type=float, help='l1 regularization')
    parser.add_argument('--tol', default=1e-2, type=float, help='integrator tolerance')
    parser.add_argument('--freq', default=100, type=int, help='message frequency')
...
def main():
    ...
    args = get_args()
    logger = get_logger(logpath=os.path.join('./pend.log'), filepath=os.path.abspath(__file__))
    ...
    data.generate_data()
    dataset = pickle.load(open("./pend.pkl", "rb" ))
    ...
    if (args.tp == 0):
        ...
        Tp = univariate_trigonometric_term().to(device)
    elif (args.tp == 2):
        Tp = univariate_trig_tri_mixed().to(device)
    else:
        Tp = univariate_fivenomial().to(device)
    if (args.vq == 0):
        Vq = univariate_trinomial().to(device)
    elif (args.vq == 1):
        Vq = univariate_trigonometric_term().to(device)
    elif (args.vq == 2):
        Vq = univariate_trig_tri_mixed().to(device)
    else:
        Vq = univariate_fivenomial().to(device)
    ...
    # ~ Save Model ~
    torch.save({'state_dict': model.state_dict()}, './pend_model.pth')
...
def train_model(model, train, test, args, device, logger):
    ...
    lr_decay = {20: 0.1, 30: 0.01, 35: 0.001}
    dt = args.dt
...
def format_data(point, dt, device):
    ...
    p0 = Variable(point[0][:, 1:2].to(device), requires_grad=True) # [[vx0, vy0]]
    q0 = Variable(point[0][:, 0:1].to(device), requires_grad=True) # [[x0, y0]]
    p1T = point[1][:, 1:2].to(device)
    q1T = point[1][:, 0:1].to(device)
