...
from function_spaces import gradient_wrapper, fivevariate_poly
...
def get_args():
    ...
    parser.add_argument('--degree', default=2, type=int, help='fivevariate polynomial degree')
    ...
    parser.add_argument('--n_epochs', default=25, type=int, help='epochs')
    parser.add_argument('--regularization', default=5e-4, type=float, help='l1 regularization')
    parser.add_argument('--tol', default=1e-2, type=float, help='integrator tolerance')
    parser.add_argument('--freq', default=200, type=int, help='message frequency')
...
def main():
    ...
    args = get_args()
    logger = get_logger(logpath=os.path.join('./ms.log'), filepath=os.path.abspath(__file__))
    ...
    data.generate_data()
    dataset = pickle.load(open("./ms.pkl", "rb" ))
    ...
    # ~ Instantiate Model ~
    Tp = fivevariate_poly(degree=args.degree).to(device) # Kinetic
    Vq = fivevariate_poly(degree=args.degree).to(device) # Potential
    ...
    # ~ Save Model ~
    torch.save({'state_dict': model.state_dict()}, './ms_model.pth')
...
def train_model(model, train, test, args, device, logger):
    ...
    lr_decay = {12: 0.1, 16: 0.01, 18: 0.001}
    dt = args.dt
...
def format_data(point, dt, device):
    ...
    p0 = Variable(point[0][:, 5:10].to(device), requires_grad=True) # [[vx0, vy0]]
    q0 = Variable(point[0][:, 0:5].to(device), requires_grad=True) # [[x0, y0]]
    p1T = point[1][:, 5:10].to(device) # [[vx1, vy1]]
    q1T = point[1][:, 0:5].to(device) # [[x1, y1]]
    return p0, q0, t0, t1, p1T, q1T

def euclideans(p1T, p1N, q1T, q1N):
    with torch.no_grad():
        true_q1, true_q2, true_q3, true_q4, true_q5 = q1T[0][0], q1T[0][1], q1T[0][2], q1T[0][3], q1T[0][4]
        true_p1, true_p2, true_p3, true_p4, true_p5 = p1T[0][0], p1T[0][1], p1T[0][2], p1T[0][3], p1T[0][4]
        pred_q1, pred_q2, pred_q3, pred_q4, pred_q5 = q1N[0][0], q1N[0][1], q1N[0][2], q1N[0][3], q1N[0][4]
        pred_p1, pred_p2, pred_p3, pred_p4, pred_p5 = p1N[0][0], p1N[0][1], p1N[0][2], p1N[0][3], p1N[0][4]
        distance_euclidean = math.sqrt((pred_q1-true_q1)**2 +
                                 (pred_q2-true_q2)**2 +
                                 (pred_q3-true_q3)**2 +
                                 (pred_q4-true_q4)**2 +
                                 (pred_q5-true_q5)**2)
        momentum_euclidean = math.sqrt((pred_p1-true_p1)**2 +
                                 (pred_p2-true_p2)**2 +
                                 (pred_p3-true_p3)**2 +
                                 (pred_p4-true_p4)**2 +
                                 (pred_p5-true_p5)**2)
